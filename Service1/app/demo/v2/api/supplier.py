...
class Supplier(Resource):
    ...
    def post(self):
        ...
        connc = conn.cursor()
        connc.execute("SELECT * FROM supplier WHERE email = '"+ userinfo["email"]+"'")
        res = connc.fetchall()
        if len(res)!=0:
            return "User has existed",200,None
        connc.execute("INSERT INTO supplier (sid,last_name,first_name,birthday,email,password,mobile_phone, userStatus) VALUES (?,?,?,?,?,?,?,?)"
